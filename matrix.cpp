...
public:
    ...
    //handles pushing a row into your matrix
    void mtxPushBack(vector<string> row){
        mtx.push_back(row);
    }
    //returns an int of the size of the matrix, which is the number of rows
    int mtxSize(){
        return (mtx.size());
    }
    //returns a string of the value at the current row and column passed in as parameters
    string &mtxGetVal(int r, int c){
        return(mtx[r][c]);
    }
    //returns the whole vector string row at the row index passed in as a parameter.
    vector<string> &mtxGetRow(int r){
        return(mtx[r]);
    }
    //addition operator overload to handle matrix addition
    Matrix operator+(Matrix &mtxz){
        Matrix tempmtx;
        for (int i = 0; i < mtx.size(); i++) {
            vector<string> row;
            for (int j = 0; j < mtx[i].size(); j++) {
                int mtxint = stoi(mtx[i][j]);
                int mtxzint = stoi(mtxz.mtxGetVal(i,j));
                row.push_back(to_string(mtxint + mtxzint));
            }
            tempmtx.mtxPushBack(row);
        }
        return(tempmtx);
    }
    //multiplication operator overload to handle matrix multiplication
    Matrix operator*(Matrix &mtxz){
        Matrix outputMtx;
        ...
        for (int i = 0; i < mtx.size(); i++){
            ...
            for (int j = 0; j < mtx.size(); j++){
                ...
                for (int k = 0; k < mtx.size(); k++){
                    ...
                    int mtx1int = stoi(mtx[i][k]);
                    int mtx2int = stoi(mtxz.mtxGetVal(k,j));
                    sum+= mtx1int*mtx2int;
                }
...
void mtxDiagonalSum(Matrix &mtxz){
    ...
    for (int i = 0; i < mtxz.mtxSize(); i++) {
        ...
        int mtxzint1 = stoi(mtxz.mtxGetVal(i,i));
        //gets secondary diagonal integer
        int mtxzint2 = stoi(mtxz.mtxGetVal(i,mtxz.mtxSize()-1-i));
...
void mtxRowSwap(Matrix &mtxz, int row1, int row2){
    ...
    vector<string> temp = outputMtx.mtxGetRow(row1);
    //sets row1 to row2
    outputMtx.mtxGetRow(row1) = outputMtx.mtxGetRow(row2);
    //sets row2 to the temp, which is row1
    outputMtx.mtxGetRow(row2) = temp;
    ...
    for (int i = 0; i < mtxz.mtxSize(); i++) {
        ...
        for (int j = 0; j < mtxz.mtxSize(); j++) {
            ...
            //if it is any other index, simply pass forward the value from mtxz into the outputMtx
                ...
                newrow.push_back(mtxz.mtxGetVal(i,j));
            }
        }
        outputMtx.mtxPushBack(newrow);
    }
...
bool boundchecker(Matrix &mtxz, int b1, int b2){
    return((b1>=0 && b1<mtxz.mtxSize()) && (b2>=0 && b2<mtxz.mtxSize()));
}
//main function
int main(){
    Matrix mtx1;
    ...
    cin >> task3mtx;
    if (task3mtx == "matrix1"){
        Matrix multmtx = mtx1*mtx2;
        multmtx.print();
    }else if (task3mtx == "matrix2"){
        Matrix multmtx = mtx2*mtx1;
        multmtx.print();
